...
public class ConvertToVtl2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Translation> listTrans = objectMapper.readValue(new File("E:\\Data\\Backup\\Lavoro\\Aubay\\BCE\\VTL\\Doc\\transformation\\transformations2.json"), new TypeReference<List<Translation>>() {});
	...
	private static void fillCustomTransformation(HashMap<String, String> customTransformation) {
	    customTransformation.put("P_EXTRCTN_RGSTRY_TBL_SCRTS_1_0", "SHR_CPTL_INVST_CNTRPRTY_FINREP := RGSTRY_TBL_SCRTS [filter(TYP_INSTRMNT in TYP_INSTRMNT_11)];");
            ...
            customTransformation.put("G_NT_RL_ET_PRTCTN_RCVD_AC_1_0", "PRTCTNS_PRTCTN_PRVDRS_SUB := PRTCTNS_PRTCTN_PRVDRS_E [filter (IS_PRMRY_PRTCTN_PRVDR = T)] [keep PRTCTN_ID, CNTRPRTY_ID][rename CNTRPRTY_ID to PRTCTN_PRVD_ID]");
            customTransformation.put("G_RL_ESTT_PRTCTN_RCVD_AC_1_1", "PRTCTNS_PRTCTN_PRVDRS_SUB := PRTCTNS_PRTCTN_PRVDRS_E [filter (IS_PRMRY_PRTCTN_PRVDR = T)] [keep PRTCTN_ID, CNTRPRTY_ID][rename CNTRPRTY_ID to PRTCTN_PRVD_ID];");
            ...
            customTransformation.put("G_IS_CMPNNT_LGL_ENTTY_1_1", "/*Extraction of all Counterparty identifiers (CNTRPRTY_ID) that are considered as components of the legal entity*/CNTRPRTY_CMPNNTS := CNTRPRTY_CMPNNTS [keep CMPNNT_ID][rename CMPNNT_ID to CNTRPRTY_ID ];");
            ...
            customTransformation.put("G_Z_06_00_REF_UNFLDD_RESL_1_0", "GRP_RESL := GRP_E [filter (RLVNT_LE = \"1\" )] [keep DT_RFRNC, GRP_ID,  LGL_ENTTY_ID, NM_DGS, NM_IPS, PRSPCTV_ID];");
            ...
            customTransformation.put("P_PSTS_THT_LBLTS_RESL_E_INTR_GRP_1_0", "GRP_RESL_INTR_GRP := GRP_E[filter (RLVNT_LE = \"1\")][ filter (GRP_ID = OBSRVD_AGNT_INTRNL_ID)][ keep DT_RFRNC, GRP_ID, LGL_ENTTY_ID, PRSPCTV_ID];");
            ...
            customTransformation.put("P_DPSTS_THT_LBLTS_RESL_1_0", "/* Create a dataset containing all Deposit Liabilities*/   DPSTS_THT_LBLTS_RESL:= DPSTS_THT_LBLTS_E [filter (TYP_INSTRMNT <> \"300\") ][keep DT_RFRNC, OBSRVD_AGNT_INTRNL_ID, INSTRMNT_UNQ_ID, PRSPCTV_ID, SBRDNTD_DBT, EXCLDD_FRM_BL_IN, DGS_ELGBL, DGS_DPST, CRRNCY_DNMNTN, OTSTNDNG_NMNL_AMNT, OTSTNDNG_NMNL_AMNT_CVRD, OTSTNDNG_NMNL_AMNT_SCRD, ERLY_RDMPTN_DT, PRCNTG_RDMPTN, DT_LGL_FNL_MTRTY, GVRND_EU_LW, RDMPTN_NTC_PRD, ADDTNL_PRTCTN];");
            ...
            customTransformation.put("P_PRTCTNS_RCVD_RESL_1_1", "PRTCTNS_RL_ESTT_RESL := PRTCTNS_RL_ESTT_E [keep PRTCTN_ID, TYP_PRTCTN][calc FNNCL_GRNT_CNSDRD_MX_AMNT := null ];");
